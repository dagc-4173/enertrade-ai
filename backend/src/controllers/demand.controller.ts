...
export function createDemandRouter(service = createDemandService(), auth = requireAuth()) {
  ...
  router.patch('/:id', express.json({ limit: '8kb' }), async (req, res, next) => { try { res.json({ demand: await service.update(req.authUser!.id, req.params.id, req.body) }); } catch (error) { next(error); } });
  router.post('/:id/cancel', express.json({ limit: '1kb' }), async (req, res, next) => { try { if (Object.keys(req.body ?? {}).length !== 0) throw new EnergyMarketInputError('INVALID_MARKET_REQUEST', 'La solicitud no debe incluir cuerpo.'); res.json({ demand: await service.cancel(req.authUser!.id, req.params.id) }); } catch (error) { next(error); } });
