import * as priceProfile from './dataset-preparation-xm-preciobolsnaci.profile';
...
export async function prepareDataset(id: number) {
  ...
  if (!['generacion', 'demanda', 'precios'].includes(dataset.dataType)) throw new DatasetPreparationError(422, 'PREPARATION_PROFILE_NOT_APPLICABLE', 'El perfil no aplica al tipo de dataset.');
  ...
  const selected = dataset.dataType === 'precios' ? (isPlainObject(report) && report.rulesetId === priceProfile.sourceRulesetId && report.rulesetVersion === priceProfile.sourceRulesetVersion ? priceProfile : null) : dataset.dataType === 'demanda' ? (isPlainObject(report) && report.rulesetId === demandProfile.sourceRulesetId && report.rulesetVersion === demandProfile.sourceRulesetVersion ? demandProfile : null) : isPlainObject(report) && report.rulesetId === sourceRulesetId && report.rulesetVersion === sourceRulesetVersion
