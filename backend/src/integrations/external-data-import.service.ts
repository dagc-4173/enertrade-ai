...
export async function importExternalData(input: unknown, service: ExternalDataService) {
  ...
  if (input.dataset !== 'Gene' && input.dataset !== 'DemaSIN') throw new ExternalDataError(400, 'UNSUPPORTED_EXTERNAL_DATASET', 'Solo se admite importar Gene o DemaSIN.');
  ...
  const source = `XM/SINERGOX;metric=${input.dataset};unit=${result.unit};startDate=${result.startDate};endDate=${result.endDate};mapping=${demand ? 'xm-demandasin-v1' : 'xm-gene-v1'}`;
  try {
    return await registerDataset({
      source, dataType: demand ? 'demanda' : 'generacion',
      columns: (demand ? ['fecha_xm', 'demanda_kwh'] : ['fecha_xm', 'hora_xm', 'energia_kwh']).map(name => ({ name, optional: false })),
      records: result.records.map(record => demand ? {fecha_xm: record.date, demanda_kwh: record.value} : ({ fecha_xm: record.date, hora_xm: record.hour, energia_kwh: record.value })),
    });
  } catch {
